...
public class Onewayplat : MonoBehaviour {
  ...
  void OnTriggerExit2D (Collider2D other){
		//Debug.Log ("JUMP RESETSSSSSSSSSSSSSSS");
		//Debug.Log (System.DateTime.UtcNow.ToString("HH:mm:ss.fff") + "Trigger is exited");
    GameObject Player = other.gameObject;
    if (Player.transform.position.y > this.transform.position.y +1.1) {
      Physics2D.IgnoreLayerCollision (0, 2, false);
			this.GetComponent<PlatformerCharacter2D>().jumpCheck= GetComponent<PlatformerCharacter2D>().jumpCheck - 2;
      this.GetComponent<Collider2D>().isTrigger = false;
    }
